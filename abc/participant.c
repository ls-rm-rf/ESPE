...
static int recv_com_and_points(conn_t* c, int* out_m, int* out_k, uint64_t** out_r){
    ...
    size_t line_size = (size_t)m * 24 + 64;
    char* points_line = (char*)malloc(line_size);
    if(!points_line) return -1;
    
    if(recv_line(c, points_line, line_size) <= 0) {
        free(points_line);
        return -1;
    }
...
int main(int argc, char** argv){
    ...
    if(a.role==1){
        // 计算方 C - 使用动态分配支持无限数量的系数
        uint64_t* coeff = (uint64_t*)calloc(10000, sizeof(uint64_t));
        if(!coeff) {
            fprintf(stderr, "Failed to allocate coefficient array\n");
            return 1;
        }
        
        int s = split_csv_int64(a.D, coeff, 10000);
        if(s<=0) {
            free(coeff);
            return 1;
        }
        ...
        for(int i=0;i<a.N;i++){
            ...
            for(int j=0;j<m;j++){
                ...
                if(recv_all(&peers[i], &rj, sizeof(rj))) {
                    free(coeff);
                    return 1;
                }
                if(recv_all(&peers[i], &vij, sizeof(vij))) {
                    free(coeff);
                    return 1;
                }
                if(recv_all(&peers[i], &siglen_net, 4)) {
                    free(coeff);
                    return 1;
                }
                ...
                if (recv_all(&peers[i], c0, 32)) {
                    free(coeff);
                    return 1;
                }
                if (recv_all(&peers[i], key_image, 32)) {
                    free(coeff);
                    return 1;
                }
                if (recv_all(&peers[i], &sig_n_net, 4)) {
                    free(coeff);
                    return 1;
                }
